...
function MonoTable(initPage) {
    ...
    var add_id_column = function () {
        if (myTable.columns.length < 9) {
            ...
            var idCol = myTable.columns.add(LocationOptions.AT_END, undefined, {
                width: colWidth
            });
    ...
    var choose_id_dialog = function (rows) {
        ...
        w.add(
            "statictext",
            undefined,
            "keine passende Tabellenzeile gefunden. Diese Zeile aktualisieren:"
        );
        ...
        for (var i = 0; i < rows.length; i++) {
            radio_group.add(
                "radiobutton",
                undefined,
                rows[i].cells.item(columnOrder["id"]).contents
            );
        }
...
exports = module.exports = MonoTable;
