function MonoPrint (aFile, baseFolder) {
	...
	if(parentFolder.name.indexOf('Druckdaten') != -1) {
		var printsFolder = parentFolder.name;
	...
	if(this.tech == 'SD' && !this.film) this.film = this.get_tagged_files('druckdaten-sd', '*'+this.tag+'_Film.indd');
	if(this.tech == 'SD' && !this.film) this.film = this.choose_file_manually('Choose Film.indd to ' + aFile.name);
