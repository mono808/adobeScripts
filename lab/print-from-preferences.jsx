...
function set_prop(prop, regEx) {
    var myList = myDocument.printPreferences[prop + 'List'];
    var myObj;
    for (var i=0, len=myList.length; i < len ; i++) {
        myObj = myList[i];
        $.writeln(myObj);
        $.writeln(regEx);
        if(myObj.match(regEx)) {
            myDocument.printPreferences[prop] = myObj;
            return myObj;
        }
    }
}

function print (myDoc, myPrinter, myPPD) {
    with(myDoc.printPreferences){
        ...
        try{
            colorOutput = ColorOutputModes.COMPOSITE_LEAVE_UNCHANGED;
...
var printer = set_prop('printer', /(Farbe ES8473)/);
//var ppd = set_prop('ppd', '/Adobe PDF/');
var paperSize = set_prop('paperSize', /A4/);
// var ppd = get_ppd('/Adobe PDF/');

print(myDocument, printer);